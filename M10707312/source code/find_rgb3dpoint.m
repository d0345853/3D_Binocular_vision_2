...
%開啟檔案以存取3D點
fid_get3d = fopen('RGB_3Dpoint.txt','wt');
%抓取左測彩圖以獲取對應rgb
image = imread('L_Color.JPG');
%抓取3D點
for i=1:len1
        ...
        LP1=F*x_left';
             ...
             store_min=ones(data_col_number_R(i),1);
              ...
              x_final_right=[data_R(p,1) data_R(p,2) data_R(p,3)];
                   ...
                   X_rgb_final=[x_result double(r) double(g) double(b)];
                   fprintf(fid_get3d,'%g\t',X_rgb_final);
                   fprintf(fid_get3d,'\n');
